...
class DynaNetGNN(nn.Module):
    ...
    def __init__(self, args, use_gpu=True, drop_prob=0.2):
        ...
        if args.en_model == 'gru':
            ...
            self.model_infer_node_agg = CNNet(
                7 if args.env == 'Ball' else 3, #origin: Ball - 7 / Cloth - 3
                nf + 2 + args.action_dim, nf * 4, nf)
    ...
    def graph_inference(self, kp, action=None, graph_gt=None, hard=False, env=None):
        ...
        # graph_gt:
        #   rel_type_gt: B x n_kp x n_kp x edge_type_num
        #   rel_attr_gt: B x n_kp x n_kp x edge_attr_dim
        args = self.args
        B, T, n_kp, _ = kp.size()
        nf = self.args.nf_hidden * 4
        ...
        if action is not None:
            ...
            if env in ['Ball', 'Cloth']:
                ...
                raise AssertionError("dicarded")
                node_rep = torch.cat([
                    node_rep, action[:, None, :, :].repeat(1, n_kp, 1, 1).view(B * n_kp, T, action_dim)], 2)
                edge_rep = torch.cat([
                    edge_rep, action[:, None, :, :].repeat(1, n_kp * n_kp, 1, 1).view(B * n_kp**2, T, action_dim)], 2)
        ...
        # edge_type = F.softmax(edge_type_logits[:, -1])
        if graph_gt is not None:
            edge_type = graph_gt[0]
    ...
    def dynam_prediction(self, kp, graph, action=None, eps=5e-2, env=None):
        ...
        edge_enc = torch.cat([
            kp_edge, edge_enc, edge_attr.reshape(B, n_kp**2, 1, edge_attr_dim).repeat(1, 1, n_his, 1)], 3)
        ...
        if action is not None:
            if env in ['Ball', 'Cloth']:
                ...
                node_enc = torch.cat([
                    node_enc, action.view(B, 1, 1, -1).repeat(1, n_kp, n_his, 1)], 3)
                edge_enc = torch.cat([
                    edge_enc, action.view(B, 1, 1, -1).repeat(1, n_kp**2, n_his, 1)], 3)


        # node_enc: B x n_kp x nf
        # edge_enc: B x n_kp x n_kp x nf
        node_enc = self.model_dynam_node_forward(
            node_enc.view(B * n_kp, n_his, -1))
        node_enc = node_enc.view(B, n_kp, nf)
        ...
        if action is not None:
            if env in ['Ball', 'Cloth']:
                ...
                raise AssertionError("discarded")
                node_enc = torch.cat([node_enc, action[:, -1:, :].repeat(1, n_kp, -1)], 2)
                edge_enc = torch.cat([edge_enc, action[:, -1:, None, :].repeat(1, n_kp, n_kp, -1)], 3)
