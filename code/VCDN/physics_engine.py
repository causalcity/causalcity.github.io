...
if __name__ == '__main__':
    ...
    if args.env == 'Ball':
        ...
        actions = np.zeros((time_step, n_ball, 2))

        act = np.zeros((n_ball, 2))
        for i in range(time_step):
            states[i] = engine.get_state()
            act += (np.random.rand(n_ball, 2) - 0.5) * 600 - act * 0.1 - states[i, :, 2:] * 0.1
            act = np.clip(act, -1000, 1000)
            engine.step(act)
            actions[i] = act

        engine.render(states, actions, engine.get_param(), video=True, image=True, path='test/Ball')
        engine.render(states, actions, engine.get_param(), video=True, image=True, path='test/Ball_inv', draw_edge=False)
