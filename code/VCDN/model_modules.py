...
class PropNet(nn.Module):
    ...
    def forward(self, node_rep, edge_rep=None, edge_type=None, start_idx=0,
        ...
        for i in range(start_idx, self.edge_type_num):
            edge_enc = self.edge_encoders[i](tmp.view(B * N * N, tmp.size(-1))).view(B, N, N, 1, self.nf_hidden)
        ...
        for i in range(self.pstep):
            ...
            for j in range(start_idx, self.edge_type_num):
                edge_effect = self.edge_propagators[i][j](tmp.view(B * N * N, tmp.size(-1)))
            ...
            node_effect = self.node_propagator(tmp.view(B * N, tmp.size(-1))).view(B, N, self.nf_hidden)

        node_effect = torch.cat([node_effect, node_enc], 2).view(B * N, self.nf_hidden*2)
        edge_effect = torch.cat([edge_effect, edge_enc], 3).view(B * N * N, edge_effect.size(-1)*2)
