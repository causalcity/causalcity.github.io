...
#    prior = np.array([0.95, 0.05])  # TODO: hard coded for now
    prior = args.prior
    print("Using prior")
    print(prior)
...
def train(epoch, best_val_loss):
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        if batch_idx % args.log_per_iter ==0:
            writer.add_scalar("Loss/nll_train", loss_nll.item(), batch_idx + (epoch*len(train_loader)))
            writer.add_scalar("Loss/kl_train", loss_kl.item(), batch_idx + (epoch*len(train_loader)))
            writer.add_scalar("Loss/mse_train", F.mse_loss(output, target).item(), batch_idx + (epoch*len(train_loader)))
        print('Epoch: {:04d}'.format(epoch),
              'Iter: {:04d}'.format(batch_idx),
              'nll_train: {:.10f}'.format(loss_nll.item()),
              'kl_train: {:.10f}'.format(loss_kl.item()),
              'mse_train: {:.10f}'.format(np.mean(F.mse_loss(output, target).item())))
