...
def load_data(rootdir, batch_size=1, suffix=''):
    loc_train = np.load(os.path.join(rootdir, suffix, 'loc_train_airsim_final.npy'))
    vel_train = np.load(os.path.join(rootdir, suffix, 'vel_train_airsim_final.npy'))
    edges_train = np.load(os.path.join(rootdir, suffix, 'edges_train_airsim_final.npy'))

    loc_valid = np.load(os.path.join(rootdir, suffix, 'loc_valid_airsim_final.npy'))
    vel_valid = np.load(os.path.join(rootdir, suffix, 'vel_valid_airsim_final.npy'))
    edges_valid = np.load(os.path.join(rootdir, suffix, 'edges_valid_airsim_final.npy'))

    loc_test = np.load(os.path.join(rootdir, suffix, 'loc_test_airsim_final.npy'))
    vel_test = np.load(os.path.join(rootdir, suffix, 'vel_test_airsim_final.npy'))
    edges_test = np.load(os.path.join(rootdir, suffix, 'edges_test_airsim_final.npy'))
    
    # [num_samples, num_timesteps, num_dims, num_atoms]
    # [num_samples, num_timesteps, num_atoms, num_dims]
    num_atoms = loc_train.shape[2]
    ...
    loc_train = np.transpose(loc_train, [0, 2, 1, 3])[:,:,:100,:2]
    vel_train = np.transpose(vel_train, [0, 2, 1, 3])[:,:,:100,:]
    feat_train = np.concatenate([loc_train, vel_train], axis=3)
    edges_train = np.reshape(edges_train, [-1, num_atoms ** 2])
    edges_train = np.array((edges_train + 1) / 2, dtype=np.int64)

    loc_valid = np.transpose(loc_valid, [0, 2, 1, 3])[:,:,:100,:2]
    vel_valid = np.transpose(vel_valid, [0, 2, 1, 3])[:,:,:100,:]
    feat_valid = np.concatenate([loc_valid, vel_valid], axis=3)
    edges_valid = np.reshape(edges_valid, [-1, num_atoms ** 2])
    edges_valid = np.array((edges_valid + 1) / 2, dtype=np.int64)

    loc_test = np.transpose(loc_test, [0, 2, 1, 3])[:,:,:,:2]
